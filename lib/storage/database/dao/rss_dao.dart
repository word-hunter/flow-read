...
class RssDao extends DatabaseAccessor<AppDatabase> with _$RssDaoMixin {
  ...
  Future<void> updateLastFetched(String url, DateTime fetchedAt) {
    ...
    return (update(rssSubscriptions)..where((r) => r.url.equals(url)))
        .write(RssSubscriptionsCompanion(lastFetchedAt: Value(iso)));
  }
  ...
  Future<Set<String>> readArticleIds() async {
    final rows = await (select(rssArticles)..where((r) => r.isRead.equals(true)))
        .get();
  ...
  Future<Set<String>> favoriteArticleIds() async {
    final rows =
        await (select(rssArticles)..where((r) => r.isFavorite.equals(true)))
            .get();
    return rows.map((r) => r.id).toSet();
  }
  ...
  Future<Set<String>> readLaterArticleIds() async {
    final rows =
        await (select(rssArticles)..where((r) => r.isReadLater.equals(true)))
            .get();
    return rows.map((r) => r.id).toSet();
  }

  Future<void> markRead(String articleId) =>
      (update(rssArticles)..where((r) => r.id.equals(articleId)))
          .write(const RssArticlesCompanion(isRead: Value(true)));

  Future<void> markUnread(String articleId) =>
      (update(rssArticles)..where((r) => r.id.equals(articleId)))
          .write(const RssArticlesCompanion(isRead: Value(false)));

  Future<void> toggleFavorite(String articleId, bool favorite) =>
      (update(rssArticles)..where((r) => r.id.equals(articleId)))
          .write(RssArticlesCompanion(isFavorite: Value(favorite)));

  Future<void> toggleReadLater(String articleId, bool readLater) =>
      (update(rssArticles)..where((r) => r.id.equals(articleId)))
          .write(RssArticlesCompanion(isReadLater: Value(readLater)));
}
